...
\begin{lstlisting}[caption=Fragment obiektów w problemie Zadania 1, label=lst:obj_task1]
...
Celem było dostarczenie paczki \texttt{p1} do Nowego Jorku, minimalizując całkowity koszt.
\begin{lstlisting}[caption=Cel i metryka w problemie Zadania 1, label=lst:goal_task1]
(:goal (at-pkg p1 newyork))
(:metric minimize (total-cost))
\end{lstlisting}

\subsection{Wygenerowany Plan}
Testy przeprowadzono w edytorze online \url{https://editor.planning.domains} z użyciem solvera \texttt{Temporal Fast Downward planning system}. Poniżej znajduje się fragment wygenerowanego planu dla finalnej wersji modelu (z czasem trwania i efektami warunkowymi).
...
\end{lstlisting}
Całkowity koszt planu dla ścieżki paczki \texttt{p1} wyniósł 174 jednostki, a czas wykonania (makespan) 11.061 jednostek czasu.
