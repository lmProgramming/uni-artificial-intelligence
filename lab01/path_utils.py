...
def generate_path(start: str, path_taken: list[models.CommunicationStep], elapsed: float, cost: float) -> models.Path:
    ...
    path_steps.append(models.LineStep(start, "", step.line, step.departure_time, none_time))
    ...
    for i, step in enumerate(path_taken[:-1]):
        ...
        path_steps.append(models.LineStep(step.start_stop.name, "", last_line, step.departure_time, none_time))   
