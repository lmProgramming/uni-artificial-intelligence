...
unique_steps = set()
for row in rows:
    step: models.CommunicationStep = models.CommunicationStep.from_parsed_csv_line(list(row.split(separator)))
...
for node in unique_steps:
    plt.scatter(node.location[1], node.location[0], color="red", s=10, label=node.name)
